...
def load_monthly_index_data(index_name, TRAIN_START_DATE, TEST_END_DATE):
    file_path = os.path.join(BASE_DIR, "index_data", indices_wide[index_name])
...
def wide_lstm_load_monthly_data(standardized, TRAIN_START_DATE, TRAIN_END_DATE, VALID_START_DATE, VALID_END_DATE, TEST_START_DATE, TEST_END_DATE):
    # Load data for all selected indices
    dfs = {index_name: load_monthly_index_data(index_name, TRAIN_START_DATE, TEST_END_DATE) for index_name in selected_indices.values()}
    ...
    # excluded_columns = ["Log_Returns_Next_Month", "Open", "High", "Low", "Adjusted_close", "Volume"]
    excluded_columns = ["Log_Returns_Next_Month"]

    feature_columns = [col for col in df_combined.columns if not any(column in col for column in excluded_columns)]
    target_columns = [col for col in df_combined.columns if "Log_Returns_Next_Month" in col]
